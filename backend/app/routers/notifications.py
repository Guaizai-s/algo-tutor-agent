...
from app.schemas.notification import NotificationListResponse, NotificationRead, RecommendationResponse
from app.services.push import get_notifications, get_recommendations, mark_all_read, mark_read
...
async def api_get_recommendations(
    ...
    """基于薄弱知识点推荐未 AC 的题目（Task 12 智能推送引擎）。

    按薄弱程度排序，每个知识点最多返回 max_per_knowledge 题（简单优先）。
    """
    return await get_recommendations(db, user_id, max_per_knowledge, max_knowledge_points)
