"""数据清洗层：常量配置 + 纯函数 + KnowledgePointSpec 构建。

本模块不依赖数据库，仅负责把 ``data/processed/combined_index.json`` + 各源 markdown
解析为 ``KnowledgePointSpec`` 列表，供 ``seed_knowledge_graph`` 模块做 DB upsert。
...
class KnowledgePointSpec:
    ...
    def __init__(self, slug: str, name: str, category: str):
        ...
        # 关联的讲义：(level, title, content, source) 四元组
...
def build_specs(
    ...
    root_specs 是 14 个一级分类虚拟根，leaf_specs 是实际知识点（含 subtag 虚拟节点），
    subtag_slug_map 是 {(category, sub_name): sub_slug} 映射，用于真实叶子挂载到二级节点。
    """
    ...
    for e in entries:
        ...
        cat_counter[category] += 1
        leaf.order = root_by_cat[category].order + cat_counter[category]
        # 讲义：按 level 映射，来源 ZUO_LECTURE
        # 只读一次 markdown，复用给 body 提取与模板代码提取
        md_text = read_markdown(e["rel_path"])
        body = extract_body(md_text)
        if body.strip():
            leaf.lectures.append((level_map[1], clean_zuo_title(e["title"]), body, LectureSource.ZUO_LECTURE))
