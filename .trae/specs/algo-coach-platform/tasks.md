...
- [ ] Task 2: 用户认证与档案
  ...
  - [ ] 2.2 实现 ACM 档案字段（学校、CF handle、AtCoder handle、目标奖牌梯度）
  - [ ] 2.3 实现 CF handle 绑定与验证（调 CF API `user.info`）
  - [ ] 2.4 构建登录/注册/个人中心前端页面
...
- [ ] Task 8: Codeforces 数据同步
  - [ ] 8.1 实现服务端 CF API 客户端（含速率限制 1 次/2 秒 + 指数退避）
  - [ ] 8.2 实现 `problemset.problems` 每日全量同步任务（Celery）
  - [ ] 8.3 实现 `user.status` 增量同步任务（每用户每 5 分钟一次）
  - [ ] 8.4 实现 `user.rating` 每日同步任务
  - [ ] 8.5 设计 Submission 表（只存元数据，不存源代码）

- [ ] Task 9: 水平测试与冷启动
  - [ ] 9.1 实现 CF 用户的冷启动逻辑（拉 user.status + cf_tags 映射 + mastery 计算）
  - [ ] 9.2 实现未绑 CF 用户的诊断题测试（15 道覆盖 10 知识点）
  - [ ] 9.3 实现起点定标算法（已学最远后代 / 薄弱节点 / 下一可学节点）
  - [ ] 9.4 按 CF Rating 定训练目标（铜/银/金/高级）
  - [ ] 9.5 构建水平测试引导流程前端页面

## Phase 4: 核心教学

- [ ] Task 10: 学习路径与推送引擎
  - [ ] 10.1 实现学习路径生成算法（前置 DAG 拓扑排序 + 跳过已掌握 + 保留补漏点）
  - [ ] 10.2 实现路径动态调整（连续 WA 3 次插入补漏任务）
  - [ ] 10.3 实现当日任务生成（1 讲义 + 1 模板题 + 2 应用题 + 1 挑战题）
  - [ ] 10.4 实现推送题目查询逻辑（按 cf_rating 升序，排除已 AC）
  - [ ] 10.5 构建当日任务卡片前端页面

- [ ] Task 11: 学习进度与掌握度
  - [ ] 11.1 设计学习记录数据模型（知识点 mastery、打卡记录）
  - [ ] 11.2 实现掌握度计算（mastery = AC 题数 / 关联题目总数）
  - [ ] 11.3 实现进度面板 API（已完成知识点数 / 通过率 / 打卡 / 雷达图 / Rating 曲线）
  - [ ] 11.4 实现弱项诊断（mastery < 0.5 自动标记）
  - [ ] 11.5 构建个人进度面板前端页面

- [ ] Task 12: 错题本
  - [ ] 12.1 实现错题自动收录（CF 同步到 WA/TLE/RE 时触发）
  - [ ] 12.2 实现错题本列表与详情 API
  - [ ] 12.3 实现同类题推荐（基于知识点关联 + cf_rating 升序）
  - [ ] 12.4 构建错题本前端页面
